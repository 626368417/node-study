...
// 读取文件时如果文件不存在会报错，读取的特点
// let content = fs.readFileSync('./note1.md', 'utf8'); // 同步阻塞，适合读取小文件 模块化我们可以采用readFileSync来读取文件

let exists = fs.existsSync('./note1.md'); // 此方法只用同步方法，异步方法不符合规范，所以不采用
console.log(exists);

// 对文件的操作一般采用相对路径，相对路径不明确

const path = require('path');

// __dirname 代表的是当前文件所在的目录 是一个绝对路径，而且不可变
console.log(path.join(__dirname, 'a', 'b', 'c', 'd', '.', '..', '/')); // 会根据系统的分割符 将对应的路径拼接在一起
console.log(path.resolve(__dirname, 'a', 'b', 'c', 'd')); // 根据执行的路径 解析出一个绝对路径，默认是可以变的（根据执行的路径来发生变化）


// path.resolve 和 path.join 可以互相转化，但是 resolve 不能遇到 / 会回到根路径，如果只是拼接则采用 join

console.log(path.extname('a.min.js')); // 获取后缀名
console.log(path.dirname('a/b/')); // 获取父路径 === __dirname

// vm node中的虚拟机模块 = new Function()
// node的模块化实现 是读取文件，让读到的内容让他执行

// eval 会以外层的作用域（不能用作模块化的实现，模块化要求就是 互相不影响）
let abc = 100;
...
// let fn = vm.compileFunction('console.log(abc)'); // 类似于 new Function
...
// 2) 会读取文件，会将内容包装成一个函数，最终返回 module.exports 此时r的结果就是 module.exports

require('./a')

// 1) Module._load 加载这个模块
// 2) Module._resolveFilename 处理路径为绝对路径，并且添加文件后缀
// 3) 拿到文件，看一下文件是否被缓存 Module._cache 是否缓存过，如果缓存过则直接结束
// 4) 如果没有缓存，则会 new Module(id, exports = {})；exports 是对模块的导出结果，默认为空
// 5) 对创建的模块缓存
// 6) 根据文件加载模块（给 module.exports 赋值）
// 7) 根据对应的文件后缀 做加载操作 Module._extensions['.js'](this, filename); 策略模式
// 8) 读取文件内容 fs.readFileSync(filename, 'utf8');
// 9) 将文件串执行 module._compile 编译字符串
